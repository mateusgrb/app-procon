...
def get_evaluation_by_region_year(company, year):
    '''
    This function gives the evaluation of a company based on the evaluation region and the specified year.
    1. Filters the results based on the 'str_nome_fantasia' of the company.
    ...
    #Esse filtro pode mudar.
    norte_evaluation = Reclamacao.objects.filter(str_nome_fantasia=company, regiao=NORTE, ano_calendario=year)
    nordeste_evaluation = Reclamacao.objects.filter(str_nome_fantasia=company, regiao=NORDESTE, ano_calendario=year)
    centro_oeste_evaluation = Reclamacao.objects.filter(str_nome_fantasia=company, regiao=C_OESTE, ano_calendario=year)
    sul_evaluation = Reclamacao.objects.filter(str_nome_fantasia=company, regiao=SUL, ano_calendario=year)
    sudeste_evaluation = Reclamacao.objects.filter(str_nome_fantasia=company, regiao=SUDESTE, ano_calendario=year)
